...
func sortUpdate(update []int, lessTable LessTable) {
	slices.SortFunc(update, func(pageNum1, pageNum2 int) int {
		if less := lessTable[pageNum1]; less != nil && less[pageNum2] {
			return -1
		}

		if less := lessTable[pageNum2]; less != nil && less[pageNum1] {
			return 1
		}
